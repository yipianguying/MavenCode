            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                            <form class="form-inline" role="form" style="float:left;">
                              ...
                              <button type="button" class="btn btn-warning" onclick="queryAuthProject()">
...
<script type="text/javascript">
    ...
    function pageQuery(pageIndex) {
        ...
        $.ajax({
            url : "${APP_PATH}/authproject/pageQuery.do",
            type : "POST",
            data : obj,
            beforeSend : function() {
            ...
            success : function( result ) {
                ...
                if ( result.success ) {
                    ...
                    var authProjectList = page.data;

                    var content = '';
                    $.each(authProjectList, function(i, authProject){
                        content += '<tr>';
                        content += '  <td>' + (i + 1) + '</td>';
                        content += '  <td>' + authProject.name + '</td>';
                        content += '  <td>' + authProject.remark + '</td>';
                        content += '  <td>';
                        content += "		<button type='button' onclick='window.location.href=\"${APP_PATH}/authproject/show.htm?pageNumber="+page.pageNumber+"&id="+authProject.id+"\"' class='glyphicon glyphicon-eye-open'>";
                        content += '  </td>';
                        content += '</tr>';
                    });
                    $("tbody").html(content);
