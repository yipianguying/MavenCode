...
<body>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<button type="button" class="btn btn-primary"
							style="float: right;" onclick="window.location.href='${APP_PATH}/project/add.htm'">
	...
	<script type="text/javascript">
            ...
            function changePageno( pageno ) {
            	//window.location.href = "<%=request.getContextPath() %>/project/index.htm?pageno="+pageno;
            	queryPage(pageno-1);
            }
            		...
            		success : function(result){
            			...
            			if(result.success){
            				...
            				$.each(list,function(i,n){
								...
								content+="	<td>";
								content+="		<button type='button' onclick='window.location.href=\"${APP_PATH}/project/edit.htm?pageno="+pageObj.pageNumber+"&id="+n.id+"\"' class='btn btn-primary btn-xs'>";
								content+="			<i class=' glyphicon glyphicon-pencil'></i>";
								content+="		</button>";
								content+="		<button type='button' class='btn btn-danger btn-xs' onclick='deleteProject("+n.id+",\""+n.name+"\")'>";
            				...
            				$("#Pagination").pagination(num_entries, {
            					...
            					current_page:(pageObj.pageNumber-1), //当前页,索引从0开始
            					prev_text:"上一页",
            					next_text:"下一页"
            				});
            				...
            				layer.msg("项目分页查询数据失败", {time:1000, icon:5, shift:6});
            				
            			}
            		},
            		error : function(){
            			layer.msg("项目分页查询数据错误", {time:1000, icon:5, shift:6});
            		}
            ...
            function deleteProject(id,name){
    			...
    			layer.confirm("是否要删除项目?",  {icon: 3, title:'提示'}, function(cindex){
    				...
    				$.ajax({
    					...
    					success : function(result){
    						if(result.success){
    							layer.msg("删除"+name+"项目成功!", {time:1000, icon:6}, function(){
    								queryPage(0);
    							});
    						}else{
    							layer.msg("删除"+name+"项目失败!", {time:1000, icon:5, shift:6});
    						}
    					}
    				});
            ...
            $("#batchDelete").click(function(){
            	...
            	if(checkedList.length>0){
            		layer.confirm("确定要删除吗?",  {icon: 3, title:'提示'}, function(cindex){
            			...
            			$.ajax({
            				url : "${APP_PATH}/project/batchDelete.do",
            				type : "POST",
            				data : datas ,
            				success : function(result){
            					if(result.success){
	            					layer.msg("删除项目成功!", {time:1000, icon:6}, function(){
	    								queryPage(0);
	    							});
            					}else{
            						layer.msg("删除项目失败!", {time:1000, icon:5});
            					}
            				}
            			});
        			    layer.close(cindex);
